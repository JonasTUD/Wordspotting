...
def wordspottingOneDoc():
    ...
    frames = frames.T
    desc = np.array(desc.T, dtype=np.float32)
    print desc.shape
    ...
    print "Lade Codebook und wende es auf berechnete Deskriporen an"
    ...
    #labels = scipy.cluster.vq.vq(desc, codebook)
    #labels = np.array(labels)[0,:]
    #print "Labels: " + str(np.array(labels).shape)
    print "ich glaube das dauert"
    ...
    siftsrechtsind = []     
    for seg in doc:
        xmin, xmax, ymin, ymax, text = seg
        b1 = frames[:,0] >= xmin
        b2 = frames[:,0] < xmax
        b3 = frames[:,1] >= ymin
        b4 = frames[:,1] < ymax
        b12 = np.logical_and(b1,b2)
        b34 = np.logical_and(b3,b4)
        b = np.logical_and(b12,b34)
        desc_seg = desc[b,:]
        helper = np.arange(desc.shape[0])[b]
        siftsind.append(helper)
        interestingSifts.append(b)
        #fuer links
        b1 = frames[:,0] >= xmin
        b2 = frames[:,0] < xmin + (xmax-xmin)/2
        b12 = np.logical_and(b1,b2)
        b = np.logical_and(b12,b34)
        desc_seg = desc[b,:]
        helper = np.arange(desc.shape[0])[b]
        siftslinksind.append(helper)
        #fuer rechts
        b1 = frames[:,0] >= xmin + (xmax-xmin)/2
        b2 = frames[:,0] < xmax
        b12 = np.logical_and(b1,b2)
        b = np.logical_and(b12,b34)
        desc_seg = desc[b,:]
        helper = np.arange(desc.shape[0])[b]
        siftsrechtsind.append(helper)
    ...
    #print np.array(siftsind)[1]
    
    maskAll = interestingSifts[0]
    for mask in interestingSifts:
        maskAll = np.logical_or(maskAll,mask)
    
    print "stuff"
    print np.array(maskAll).shape
    print np.array(desc).shape
    desc = np.array(desc)
    descInSegs = desc[maskAll]
    print np.array(desc).shape
    labels = scipy.cluster.vq.vq(desc, codebook)
    labels = np.array(labels)[0,:]
